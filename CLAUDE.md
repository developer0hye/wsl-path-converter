...
**IMPORTANT:** `lib/convert.ahk` is `#Include`d by both `wsl-path-converter.ahk` and `test.ahk`. When modifying conversion logic, only edit `lib/convert.ahk`. Never duplicate conversion functions — the shared include ensures main and test always use identical logic.

## CI

- `test.yml` — Runs `test.ahk` on every push/PR to master
- `release.yml` — Compiles `.exe` and creates GitHub Release on `v*` tags
